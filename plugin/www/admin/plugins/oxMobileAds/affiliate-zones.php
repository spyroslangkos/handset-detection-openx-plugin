...
+---------------------------------------------------------------------------+
$Id: affiliate-zones.php 34688 2009-04-01 16:18:28Z andrew.hill $
*/
...
$oTpl = new OA_Plugin_Template('zones-index.html');
...
$ADMINPATH = '/www/admin/';
    if ($websiteId) {
        ...
        if (OA_Permission::isAccount(OA_ACCOUNT_ADMIN, OA_ACCOUNT_MANAGER)) {
            $websiteEditUrl = $ADMINPATH. "affiliate-edit.php?affiliateid=$websiteId";
        }
    }
...
$ADMINPATH = '/www/admin/';
    ...
    while ($doAffiliates->fetch() && $row = $doAffiliates->toArray()) {
        $aWebsiteMap[$row['affiliateid']] = array('name' => $row['name'],
            'url' => $ADMINPATH. "affiliate-edit.php?affiliateid=".$row['affiliateid']);
    }
